...
public class GraphApplication {
	...
	CommandLineRunner start(CompteRepository compteRepository, TransactionRepository transactionRepository) {
		return args -> {
			...
			Compte compte1 = new Compte(null, Math.round(Math.random() * 9000 * 100) / 100.0, new Date(), TypeCompte.EPARGNE, new ArrayList<>());
			Compte compte2 = new Compte(null, Math.round(Math.random() * 9000 * 100) / 100.0 , new Date(), TypeCompte.COURANT, new ArrayList<>());
			Compte compte3 = new Compte(null, Math.round(Math.random() * 9000 * 100) / 100.0 , new Date(), TypeCompte.EPARGNE, new ArrayList<>());
